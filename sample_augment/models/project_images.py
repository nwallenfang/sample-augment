...
def project_training_subset(data: TrainSet, generator_name: str, number_per_class=20):
    ...
    for class_idx in range(G.c_dim):  # c_dim == num_classes
        ...
        for instance_idx in sample_indices:
            ...
            target_image_data = data.image_tensor[instance_idx].numpy()
            target_class_label = data.label_tensor[instance_idx]
            target_classname = GC10_CLASSES[class_idx]
            projected_fname = f"{target_classname}_{instance_idx}"

            # TODO save full label information in npz file as well !
            run_projection(
                G,
                device,
                target_image_data,
...
def main():
    ...
    data: AugmentDataset = AugmentDataset.from_file(root_dir / "AugmentDataset/dataset_f00581.json")  # on colab this works apparently
