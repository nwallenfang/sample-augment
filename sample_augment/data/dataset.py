...
def image_folder_to_tensor_dataset(image_dataset: ImageDataset,
                                   name: str = 'gc10',
                                   true_labels: dict[str, int] = None) -> AugmentDataset:
    """
    ...
    tensor_dataset = AugmentDataset(name, image_tensors, label_tensors, img_ids=img_paths,
                                    root_dir=root_dir)
...
def main():
    ...
    if not os.path.exists(project_path('data/interim/gc10_tensors.pt')):
        ...
        tensor_dataset = AugmentDataset.load_from_file(
